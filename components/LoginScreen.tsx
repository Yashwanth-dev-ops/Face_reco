    ...
    return (
        <div className="w-full max-w-sm mx-auto animate-fade-in">
            ...
            <div className="bg-slate-800/40 rounded-2xl shadow-2xl p-8 border border-slate-800 backdrop-blur-sm">
                {showFaceLogin ? (
                    <FaceLogin onLogin={onLogin} onCancel={() => setShowFaceLogin(false)} />
                ) : (
                <>
                    <div className="flex border-b border-slate-700 mb-6">
                        <button onClick={() => setLoginType('ADMIN')} className={`flex-1 py-2 font-semibold transition-colors ${loginType === 'ADMIN' ? 'text-indigo-400 border-b-2 border-indigo-400' : 'text-gray-400'}`}>Admin</button>
                        <button onClick={() => setLoginType('STUDENT')} className={`flex-1 py-2 font-semibold transition-colors ${loginType === 'STUDENT' ? 'text-indigo-400 border-b-2 border-indigo-400' : 'text-gray-400'}`}>Student</button>
                    </div>
                    {renderPasswordForm()}
                     ...
                     <button onClick={() => setShowFaceLogin(true)} className="w-full px-6 py-2.5 rounded-md font-semibold text-white bg-slate-700 hover:bg-slate-600 transition-all duration-150 ease-in-out focus:outline-none focus:ring-2 focus:ring-slate-500 active:translate-y-0.5 flex items-center justify-center shadow-md">
                        Login with Face ID
                    </button>
