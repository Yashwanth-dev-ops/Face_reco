...
const SummaryItem: React.FC<SummaryItemProps> = ({ emoji, label, confidence, colors, description }) => (
    <div className="flex items-center gap-3 p-2.5 rounded-lg bg-slate-800/60 animate-fade-in">
...
export const DetectionSummary: React.FC<{ result: DetectionResult, studentCount: number }> = ({ result, studentCount }) => {
    ...
    return (
        <div className="w-full md:w-80 lg:w-96 flex-shrink-0 p-4 space-y-4 bg-slate-800/40 rounded-2xl border border-slate-800 backdrop-blur-sm self-stretch">
            <div className="flex flex-col h-full">
                {!hasDetections && (
                     <div className="flex-grow flex flex-col items-center justify-center text-center text-gray-500">
                        ...
                        <p className="text-xs mt-4">Total registered students: {studentCount}</p>
                    </div>
                )}
                {hasDetections && (
                    <div className="overflow-y-auto space-y-6 pr-1 h-full">
                        {visibleRegisteredStudents.length > 0 && (
                             <div>
                                <h3 className="text-lg font-bold text-purple-300 mb-3">Registered Students ({visibleRegisteredStudents.length})</h3>
                                <div className="space-y-2">
                                    {visibleRegisteredStudents.map((face) => (
                                        <div key={`person-${face.persistentId}`} className="p-2.5 rounded-lg bg-slate-800/60 animate-fade-in">
